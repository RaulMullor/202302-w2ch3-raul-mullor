...
const askForNumber = () => {
  ...
  if (isNaN(nuevoNumero)) {
    if (numerosCalculadoraDiv.length < 1) {
      ...
      const suma = numerosCalculadoraSuma.reduce(
        (anteriorNumero, numeroActual) => anteriorNumero + numeroActual
      );
      const resta = numerosCalculadoraResta.reduce(
        (anteriorNumero, numeroActual) => anteriorNumero - numeroActual
      );
      const multiplicacion = numerosCalculadoraDiv.reduce(
        (anteriorNumero, numeroActual) => anteriorNumero * numeroActual
      );
      const division = numerosCalculadoraMult.reduce(
        (anteriorNumero, numeroActual) => anteriorNumero / numeroActual
      );
      if (suma % 2 === 0) {
